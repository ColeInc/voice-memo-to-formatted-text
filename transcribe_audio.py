...
class AudioTranscriber:
    ...
    def send_transcript_email(self, formatted_transcript: str, original_filename: str):
        """Send the formatted transcript as an email"""
        if not EMAIL_CONFIG['sender_email'] or not EMAIL_CONFIG['sender_password']:
            self.logger.warning("Email credentials not configured, skipping transcript email")
            return
            
        try:
            # Extract the title from the formatted transcript for the subject line
            subject = "Voice Memo Transcript"
            lines = formatted_transcript.split('\n')
            if lines and lines[0].startswith('# '):
                subject = lines[0][2:].strip()  # Remove '# ' and strip whitespace
            ...
            # Create HTML version from markdown for better email formatting
            html_body = f"""
            <html>
            <head></head>
            <body>
...
{formatted_transcript}
            ...
            msg.attach(MIMEText(formatted_transcript, 'plain'))
            ...
            self.logger.info(f"Transcript email sent successfully with subject: {subject}")
            
        except Exception as e:
            self.logger.error(f"Failed to send transcript email: {str(e)}")
    ...
    def save_transcript(self, transcript: str, original_filename: str) -> Optional[Path]:
        """Save transcript to OUTPUT directory with timestamp and send email"""
        try:
            ...
            self.send_transcript_email(formatted_transcript, original_filename)
    ...
    def format_transcript(self, transcript: str, original_filename: str) -> str:
        ...
        # Extract title from Gemini response if it starts with # heading
        main_title = "Voice Memo Transcript"
        content = gemini_formatted
        
        if self.gemini_enabled and gemini_formatted.startswith('# '):
            lines = gemini_formatted.split('\n', 1)
            if len(lines) >= 2:
                main_title = lines[0][2:].strip()  # Remove '# ' and strip whitespace
                content = lines[1].strip()  # Get the rest of the content
        
        # Create markdown formatted output with extracted title
        formatted = f"""# {main_title}
